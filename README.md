...
> 기본 종합 점수는 전체 후보 선별용이며, 단기/중기/장기 후보는 별도 점수로 분리합니다.
...
- 중기 매매 후보: 기업 데이터 30%, 시장 데이터 30%, 차트 분석 25%, 뉴스 15%로 2주~3개월 후보를 별도 랭킹
- 장기 투자 후보: 기업 데이터 45%, 시장/산업 25%, 차트 분석 15%, 구조적 이슈 15%로 3개월~1년 이상 후보를 별도 랭킹
...
기본 흐름은 `위험 제거 -> 성장+수익성 확인 -> 업종별 밸류 판단 -> 모멘텀으로 진입 타이밍 확인 -> 매도/비중관리`입니다. 점수는 매수 지시가 아니라 후보 압축용이며, 중장기 관점에서 리스크는 공격형으로 보되 지뢰성 재무 위험은 먼저 제거합니다.

- 리스크 선제 필터: 부채비율 극단값, 낮은 이자보상배율, 낮은 유동비율, 영업적자와 FCF 음수의 동시 발생은 `Hard Fail`로 분리하고 `제외` 처리합니다.
- 공격형 허용: 고성장과 현금흐름 버팀목이 동시에 있으면 일부 부채/유동성 리스크를 `Aggressive Allow`로 남기되 리스크 등급은 `높음`, 목표/최대 비중은 낮게 제한합니다.
- 업종/스타일별 가중치: 성장주는 성장 품질을, 가치/금융주는 밸류와 수익성을, 경기민감주는 산업 사이클과 밸류를 더 크게 반영합니다.
- 포트폴리오 비중: `매수 후보`는 목표 8%/최대 10%, `관심`은 목표 4%/최대 6%, `관망`은 신규 매수 없이 최대 3%, `제외`는 0%로 둡니다.
- 매도/축소 기준: `Hard Fail`, `제외` 등급, 성장·수익성 훼손, 밸류 부담과 모멘텀 둔화가 동시에 나타나면 축소 또는 제외 신호를 냅니다.
- 점검 주기: 매월 점검하되 실제 조정은 분기 중심으로 해 과잉매매를 줄입니다. 업종 상한은 35%, 종목 수는 가능하면 8~12개 이상으로 분산합니다.
...
- 검증 방식: `snapshot`은 스냅샷 Top N 동일비중, `rules`는 리스크 게이트와 등급별 목표비중을 반영한 규칙 기반 검증, `legacy`는 현재 유니버스 기반 참고 검증입니다.
